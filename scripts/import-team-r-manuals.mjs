...
function normalizePack(pack) {
  ...
  const strategy = pack.strategy
    ? {
        ...pack.strategy,
        bring: pack.strategy.bring ?? pack.slugs,
        ...(contrast && !pack.strategy.contrast ? { contrast } : {}),
      }
    : undefined;
  ...
  return {
    ...
    ...(pack.defaultLeadPair ? { defaultLeadPair: pack.defaultLeadPair } : {}),
    ...(pack.backPair ? { backPair: pack.backPair } : {}),
...
function normalizeManual(raw) {
  ...
  const pilot = {
    ...
    rule: raw.pilot?.rule ?? "Pick the package whose clock the opponent cannot answer first.",
  ...
  const loops =
    raw.loops?.length
      ? raw.loops
      : packs.flatMap((p) => p.loops ?? []).slice(0, 3).map((l) => ({
          title: l.title,
          body: l.body,
        }));
