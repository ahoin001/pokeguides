...
/**
 * Quiet constellation — labels on hover/select only.
 */
export function ManualNetworkGraph({
  ...
  const nodes = useMemo(() => {
    ...
    const positions: { slug: string; x: number; y: number }[] = [
      { slug: slugs[0] ?? "", x: 50, y: 12 },
      { slug: slugs[1] ?? "", x: 18, y: 38 },
      { slug: slugs[2] ?? "", x: 82, y: 38 },
      { slug: slugs[3] ?? "", x: 50, y: 48 },
      { slug: slugs[4] ?? "", x: 28, y: 78 },
      { slug: slugs[5] ?? "", x: 72, y: 78 },
    ];
  ...
  const edges = useMemo(() => {
    ...
    // Prefer tempo → converter style: first hop into mid/lower nodes
    ...
    >
      <div className="relative mx-auto aspect-[4/3] w-full max-w-xl">
        ...
        {nodes.map((n) => {
          ...
          return (
            <button
              ...
              className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full border border-line/70 bg-bg p-1 shadow-sm transition hover:scale-105"
        ...
        {hover !== null && edges[hover] ? (
          <div className="pointer-events-none absolute bottom-2 left-1/2 z-10 w-[min(90%,18rem)] -translate-x-1/2 rounded-2xl border border-line bg-bg/95 px-3 py-2 text-center text-xs shadow-md backdrop-blur-sm">
            <p className="font-medium text-ink">
              {getPokemon(edges[hover]!.from)?.name} → {getPokemon(edges[hover]!.to)?.name}
            </p>
            <p className="mt-0.5 text-muted">
              {edges[hover]!.creates} → {edges[hover]!.converts}
            </p>
          </div>
        ) : null}
      </div>
